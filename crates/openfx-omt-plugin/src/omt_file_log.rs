//! libomtnet-compatible file logging under `{storage}/logs/{exe}{pid}.log`.
//!
//! Storage directory (same as official `settings.xml`):
//! - Windows: `%ProgramData%\OMT` (`C:\ProgramData\OMT`)
//! - macOS / Linux: `~/.OMT`
//! - Override with `OMT_STORAGE_PATH`
...
/// Write to stderr and to the OMT log file.
#[allow(dead_code)]
pub fn eprint_and_file(message: &str) {
    eprintln!("{message}");
    write(message, "OMT");
}
